' KSGE K.I.S.S. STRIP GAME ENGINE - FREE VERSION FOR UNCRYPTED MKV CLIPS
...
' VERSION 6.3FREE 20220609 free version works with unencrypted mkv content - no monetization
...
const C1 as string = "FreeModel" 'model name wich should be equal to folder name
...
shash = "  -" 'single hash for all clip *.cpt files
shashw = "  -" 'single hash for all clip *.cpt files for windows platform
kspcha = "  kspc" ' hash for kspc
kspchaw= "kspc.exe" ' hash for kspc.exe for windows
...
raddress(1) = "-" 'address to check transaction for (where monetize)
...
rown = "Kiss Strip Poker Free" 'info about game author (mail, website, social, etc)
const C3 as string = "mkv" 'clip file format
const C2 as string = "0" 'debug 0=no 1=yes
const C4 as string = "KISS STRIP POKER FREE" 'game name
...
if currentstage > 0 then
	...
	case "off"
		...
		'6.3FREE if currentstage > C6 then chkhw 'checks if game is in demo mode
	case "end"
		clipcounter (nclipend)
		cliptoplay = nclipend + str(Int(rnd_range(1, clipcount+1))) + ncliptype 
		avoidduplicate
...
if currentstage = 0 then
   ...
   '6.3FREE cmdline = "echo " + K1 + "| " + decryptexename + " -c -k - " + mediaFileName +  " > " + tmpmediaFileName
   
	'6.3FREE if C2 = "1" then print cmdline 'debug
		
	Dim result As Integer
	'6.3FREE chkbin
	'6.3FREE result = Shell (cmdline) 
	'6.3FREE If result = -1 Then
	'6.3FREE	Print "Error running "; mediaFileName
	'6.3FREE Else
	'6.3FREE	if C2 = "1" then Print "Exit code:"; result 'debug
	'6.3FREE End If
	'return 0
   'if play(mediaFileName, pInstance, pPlayer) < 0 then exit while ' <------- HERE STARTS PLAY CLIP
   'print "playing: " ; tmpMediaFileName 'debug
   '6.3FREE if play(tmpmediaFileName, pInstance, pPlayer) < 0 then print mediaFileName, "MAYBE NOT FOUND???" ' <------- HERE STARTS PLAY CLIP
   if play(mediaFileName, pInstance, pPlayer) < 0 then print mediaFileName, "MAYBE NOT FOUND???" ' <------- HERE STARTS PLAY CLIP
   '6.3FREE kill tmpmediaFileName 'after played tmp uncrypted file is deleted
